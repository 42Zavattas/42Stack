...
exports.votesAll = function (req, res) {
	return Vote.find(function (err, things) {
		if (!err) {
			return res.json(things);
		}
		else {
...
exports.votesFromUser = function (req, res) {
	return Vote.find({ sender : req.params.login }, function (err, things) {
		if (!err) {
			return res.json(things);
		}
		else {
...
exports.votesToUser = function (req, res) {
	return Vote.find({ receiver : req.params.login }, function (err, things) {
		if (!err) {
			return res.json(things);
		}
		else {
...
exports.votesOnQuestion = function (req, res) {
	return Vote.find({ obj : req.params.objectid }, function (err, things) {
		if (!err) {
			return res.json(things);
		}
		else {
...
exports.voteAction = function (req, res) {
	...
	Question.findOne({ _id : vote.obj }, function (err, res) {
		if (!err) {
			if (!res) {
				Answer.findOne({ _id : vote.obj }, function (err, res) {
					if (!err) {
						if (!res) {
							res.send(500, 'Something bad happend, your login has been recorded');
						}
						else {
							vote.receiver = res.author;
						}
					}
					else {
				...
				if (req.body.type == result.type) {
					...
					result.save(function (err, result) {
						if (err) {
							...
							res.send(200, 'You change your vote');
						}
					});
				}
			}
